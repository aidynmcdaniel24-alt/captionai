...
type Props = {
  ...
  resetAt?: string | null;
  /** Optional headline override — defaults to the standard 200 token reset copy. */
  message?: string;
};

const PRO_BENEFITS: { icon: string; label: string }[] = [
  { icon: "♾️", label: "Unlimited tokens — generate without limits" },
...
/**
 * Premium-feeling upgrade popup shown the moment a free user hits 0
 * tokens. Replaces the old "Free Limit Reached" error and intercepts
 * EVERY feature (captions, hashtags, bio, A/B test) so it is the
 * single touchpoint between hitting a wall and converting.
 */
      ...
      >
        ...
        <div className="relative">
          ...
          <p className="mt-2 text-sm leading-relaxed text-purple-100/90 sm:text-base">
            {message ??
              "Free plan gives you 200 tokens daily. Keep the momentum going — unlock unlimited captions, analytics, image-to-caption, and more with Pro."}
          </p>
